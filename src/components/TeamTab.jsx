...
const TeamTab = () => {
    ...
    const [form, setForm] = useState({ name: '', loginId: '', password: '', role: 'TRAINER' });
    const [editingUserId, setEditingUserId] = useState(null);
    const [editUserForm, setEditUserForm] = useState({ name: '', password: '' });
    const [permissions, setPermissions] = useState([]);
    const [error, setError] = useState(null);

    const rolesList = ['DIRECTOR', 'OPERATION_HEAD', 'TECHNICAL_COORDINATOR', 'TECHNICAL_ASSOCIATE', 'TRAINER'];
    ...
    const createUser = async (e) => {
        ...
        try {
            ...
            const data = await res.json();
            if (res.ok) {
                setForm({ name: '', loginId: '', password: '', role: 'TRAINER' });
                fetchUsers();
